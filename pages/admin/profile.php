...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$fullName) {
        ...
        if (!password_verify($currentPass, $pwRow->fetchColumn())) {
        ...
        if ($upload['error']) {
            ...
            if ($newPass) {
                $db->prepare("UPDATE users SET full_name=?, phone=?, profile_image=?, password=? WHERE id=?")
                   ->execute([$fullName, $phone, $upload['path'], password_hash($newPass, PASSWORD_DEFAULT), $userId]);
            } else {
                $db->prepare("UPDATE users SET full_name=?, phone=?, profile_image=? WHERE id=?")
                   ->execute([$fullName, $phone, $upload['path'], $userId]);
            }
        ...
        <div class="col-lg-4">
            <div class="cir-card text-center">
                <div class="cir-card-body" style="padding:32px 24px;">
                    ...
                    <span class="sidebar-role-badge admin-badge">Administrator</span>
                    ...
                    <form method="POST" enctype="multipart/form-data">
                        ...
                        <div class="form-row">
                            ...
                            <p style="font-size:0.78rem;color:#94a3b8;margin-top:5px;">
                                <i class="bi bi-lock me-1"></i>Admin email is managed at the system level.
                            </p>
                        </div>
                        ...
                        <p class="fw-semibold" style="font-size:0.88rem;color:var(--cir-navy);margin-bottom:14px;">
                            <i class="bi bi-shield-lock me-1 text-green"></i>Change Password
                            <span style="font-weight:400;color:#94a3b8;">(leave blank to keep current)</span>
                        </p>
                        ...
                        <div class="d-flex gap-3 mt-2">
                            ...
                            <a href="<?= BASE_URL ?>/pages/admin/dashboard.php" class="btn-cir-outline" style="padding:11px 22px;">
                                Cancel
                            </a>
                        </div>
                    </form>
